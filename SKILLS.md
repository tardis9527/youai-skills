...
自进化的公众号写作系统：带记忆开工→全网调研→四轮采访→整理确认→风格选择→撰写初稿→六维评分自审→用户把关→配图设计与AI生图→学习记录，形成"使用→反馈→学习→优化"闭环，越用越懂你。
- 适用场景：写公众号文章、起标题大纲、去 AI 味、润色成个人文风、定稿后配图与出图
- 产出：通过六维评分与去 AI 味自审的定稿（含标题）+ 资深设计师视角的配图提示词，可直接调用 AI 生图渠道产出真实配图 + 沉淀到本地知识库的学习记录

### 9. 朋友圈文案自进化写作（Moments Writer）
自进化的朋友圈写作系统：带人设记忆开工→场景速记→人设/参数确认→批量生成候选→自查清洗→用户挑选→配图设计与AI生图→学习记录，形成"使用→反馈→学习→优化"闭环，越用越懂你的人设和语气。
- 适用场景：写朋友圈配文、发圈文案、去朋友圈"塑料感"、定稿后配图与出图
- 产出：3-5 条通过轻量检查表自审的候选文案 + 资深设计师视角的配图提示词，可直接调用 AI 生图渠道产出真实配图 + 沉淀到本地知识库的学习记录
